...
export const asyncLinkManager = (managedUser: ManagedUser) => {
  ...
  const pending = idm.managed.pendingRelationships.query({
    filter: allOf(equals("targetCollection", idm.managed.user.type), equals("targetUniqueKey", managedUser.userName), not(presence("actionTime"))),
  });
